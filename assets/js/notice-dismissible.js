...
const dismissNotice = (selector) => {
    document.querySelectorAll(selector).forEach((el) => {
        (0,_helpers_events__WEBPACK_IMPORTED_MODULE_0__.addEventListenerLive)('click', '.ac-notice__dismiss, [data-dismiss], .notice-dismiss', (e) => {
            e.preventDefault();
            let data = el.dataset.dismissibleCallback ? JSON.parse(el.dataset.dismissibleCallback) : null;
            if (data) {
                jquery__WEBPACK_IMPORTED_MODULE_1___default().post(ajaxurl, data);
            }
        }, el);
    });
};
...
document.addEventListener('DOMContentLoaded', () => {
    (0,_plugin_dismissible_notice__WEBPACK_IMPORTED_MODULE_0__.initDismissibleNotices)();
});
