	...
	{#each options as option}
		<AcRadio bind:group={selectedOption}
			...
			{#if option.value === 'wp_default' && config.input?.data[ 'wp_date_format' ] }
				<code class="acu-bg-[var(--ac-surface-subtle)]">{config.input?.data[ 'wp_date_format' ]}</code>
			{:else if config.input?.data[ 'format_codes' ]?.[ option.value ] }
				<code class="acu-bg-[var(--ac-surface-subtle)]">{config.input?.data[ 'format_codes' ][ option.value ]}</code>
			{/if}
		</AcRadio>
	{/each}
	<div class="custom">
