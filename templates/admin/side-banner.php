...
<div class="sidebox" id="ac-pro-version">
	<div class="padding-box">
		<h3>
			<a href="<?= esc_url($upgrade_page_url->add_content('title')->get_url()); ?>">
                <?php
                _e('Upgrade to', 'codepress-admin-columns'); ?>&nbsp;<span><?php
                    _e('Pro', 'codepress-admin-columns'); ?></span>
			</a>
		</h3>

		<div class="inside">
			<p><?php
                _e('Take Admin Columns to the next level:', 'codepress-admin-columns'); ?></p>
			<ul class="features">
                <?php
            ...
            if ($integrations) : ?>
				<strong class="extra"><?php
                    _e('Includes special integrations for:', 'codepress-admin-columns'); ?></strong>
				<ul>
                    <?php
			...
			<p class="center ac-pro-prices">
                ...
                if ($this->price) : ?>
                    <?php
                    printf(__('Prices starting from %s', 'codepress-admin-columns'), '$' . $this->price); ?>
                <?php
                endif; ?>
			</p>
		</div>
    ...
    if ($is_promo_active) : ?>
		...
		<div class="padding-box ac-pro-deal">
            ...
            echo esc_url($promo->get_url()->get_url()); ?>" class="acp-button">
                <?php
                echo esc_html(sprintf(__('Get %s Off!', 'codepress-admin-columns'), $promo->get_discount() . '%')); ?>
			</a>
			<p class="nomargin">
                <?php
                echo esc_html(
    ...
    <?php
		...
		<div class="padding-box ac-pro-newsletter">
			<h3>
                <?php
                echo esc_html(sprintf(__('Get %s Off!', 'codepress-admin-columns'), $discount . '%')); ?>
			</h3>
			<div class="inside">
				<p>
                    <?php
				...
				<form method="post" action="<?= esc_url(
                    ...
                    esc_attr_e("Your Email", 'codepress-admin-columns'); ?>" value="<?= esc_attr(
                        $user_data->user_email
                    ); ?>" required>
					<input type="text" name="FNAME" placeholder="<?php
                    esc_attr_e("Your First Name", 'codepress-admin-columns'); ?>" value="<?= esc_attr(
                        $user_data->first_name
                    ); ?>" required>
					<input type="submit" value="<?php
                    esc_attr_e("Send me the discount", 'codepress-admin-columns'); ?>" class="acp-button">
				</form>
			</div>
		</div>
