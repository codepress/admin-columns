...
{	
	...
	// custom field button
	$button_add_column = "<a href='javacript:;' id='cpac-add-customfield-column'>Add custom field column</a>";
...
function cpac_get_box($post_type, $key, $values) 
{
	...
	// hide box options
	if ( isset($values['hide_options']) && $values['hide_options'] ) {
		$action = $more_options = '';
	}
	...
	$list = "
		<li class='{$class}'>
			...
			<div class='cpac-type-options'>
				...
				<input type='hidden' name='cpac_options[columns][{$post_type}][{$key}][class]' value='{$values['class']}'/>
