...
class AC_Settings_Tab_Columns extends AC_Settings_TabAbstract {
	...
	private function store( AC_StorageModel $storage_model, $column_data ) {
		...
		// store columns
		$settings = new AC_Settings( $storage_model->get_key(), $storage_model->get_active_layout() );
		...
		if ( ! $result ) {
			return new WP_Error( 'same-settings', sprintf( __( 'You are trying to store the same settings for %s.', 'codepress-admin-columns' ), "<strong>" . $storage_model->get_label_or_layout_name() . "</strong>" ) );
		}
